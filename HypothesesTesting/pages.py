...
class CheckArticleSelectionWaitPage1(WaitPage):
    ...
    body_text = "Ihr Gruppenmitglied wählt gerade das Suchergebnis aus."
...
class CheckArticleSelectionWaitPage2(WaitPage):
    ...
    def is_displayed(self):
        return self.player.singleplayer == 0 and self.player.disagreement1 == 1
...
class SelectArticleAfterDisagreementPage(Page):
    form_model = "player"
    form_fields = ["selectedSR","SRbyTimeout"]
    # display page conditionally
    def is_displayed(self):
        return self.player.singleplayer == 0 and self.player.disagreement1 == 1
...
class SelectedArticlePage(Page):
    ...
    # increase article selection round
    def before_next_page(self):
        self.player.articleSelectionRoundInd += 1
        self.group.articleSelectionRound = self.player.articleSelectionRoundInd
...
class SingleplayerArticleSelectionPage(Page):
    form_model = "player"
    form_fields = ["selectedSR","articleByTimeout"]
    #display page conditionally
    def is_displayed(self):
        return self.player.singleplayer == 1
...
page_sequence = [# match group members
                 ...
                 # Reminder Page with short instructions
